...
% NeurIPS 2026 AIWILD requires fully anonymized submissions.
...
\title{Shared-Memory Honeytokens\\in Multi-Agent LLM Systems: A Controlled Simulation}

\workshoptitle{Third Workshop on Agents in the Wild: Safety, Security, and Beyond}

\makeatletter
\renewcommand{\@noticestring}{Submitted to the \@workshoptitle\ at NeurIPS 2026. Do not distribute.}
\makeatother
...
that a coalition can pool weak fingerprints to distinguish decoys.
...
$[0,0]$ while reaching the highest trap activation ($0.80$); baseline,
coalition, and rotation reach harm $12.8$, $16.0$, and $16.9$. The
non-containment intent sweep spans activation $0.20$--$1.00$ and harm
$0.4$--$15.8$; containment-high keeps harm at $0$.
Core-matrix benign false-positive rate is $0$--$3.3\%$.
...
honeytoken boundary shows how a coalition can pool weak fingerprints and
reduce its decoy-classification error as observations accumulate~\citep{gans2026agents}.

We built CanaryGame to measure that prediction. In this controlled testbed,
open-weight models act as both benign workers and
...
Containment & \textbf{0.80} & \textbf{0.00} & \textbf{0.0 (0.0, 0.0)} & 4.0 & 0.033 \\
\bottomrule
\end{tabular}
\end{table}
...
\section{Results}
\textbf{Containment blocks decoy harm (H3).} Containment alone stops all
damage despite the highest activation ($0.80$). Its attack coverage is $0.00$,
and its post-trigger harm is
exactly zero with CI $[0,0]$ (Table~\ref{tab:core}, Figure~\ref{fig:core}).
Traces show the broker refusing every decoy attempt, so attackers keep probing
but never move scored state. The same zero-harm result holds across model sizes
and in the Gemma containment condition (Section~\ref{sec:robust}).
...
pins harm at zero ($0.60$ activation, $0$ harm).
...
contain.\ high      & high   & 0.60 & \textbf{0.0 (0.0, 0.0)} & 3.0 & 0.80 \\
contain.\ medium    & medium & 0.40 & \textbf{0.0 (0.0, 0.0)} & 2.2 & 1.00 \\
\bottomrule
\end{tabular}
\end{table}

\textbf{Benign false alarms rise under high-intent attack.} Across the core
matrix, benign FPR is $0$--$3.3\%$ and benign success $0.70$--$1.00$
(Figure~\ref{fig:core}d). One trajectory-signature detector reports an
unconditional benign FPR of 25--53\%, depending on the recall
protocol~\citep{leong2026forensic};
...
Benign behavior is similar on Gemma3-4B: engagement is $0.96$--$1.00$,
...
their first credential around step 21--39 (Qwen: step 3). The original Gemma
matrix activates traps in $0.00$--$0.10$ of episodes; the corrected private-
namespace containment rerun reaches $0.40$ at 80 steps (Figure~\ref{fig:robust}).
Where Gemma attackers do misuse decoys, containment blocks them all (harm $0.0$).
Qwen3-8B coalition harm is $1.9$ versus $16.0$ for Qwen3-4B, and containment
again holds harm at exactly zero with CI $[0,0]$.
...
Sharing raises mean harm and spreads fingerprints, although the harm CIs overlap.
A private reference monitor keyed on decoy tokens yields zero post-trigger harm
and keeps benign FPR low (3.3\% in the corrected core run). Removing decoys from
the broker's issuance path makes every decoy touch a dead end. Rotation lowers
activation but leaves harm high after contact.
...
The full dataset (60 records, fixed seeds, acceptance-checked) will accompany
the submission as anonymized supplementary material and be released publicly
after review; code, configs, pod runner scripts, and per-condition reports
accompany it.
...
\bibitem[Gans(2026)]{gans2026agents}
J.~S. Gans.
\newblock When agents talk: Honeytokens under shared memory.
\newblock \emph{arXiv:2608.11436}, 2026.

\bibitem[Leong(2026)]{leong2026forensic}
J.~W. Leong.
\newblock Forensic trajectory signatures for agent memory poisoning detection.
\newblock \emph{arXiv:2606.30566}, 2026.
...
both sizes; Qwen3-8B has lower coalition activation and harm.}
\label{fig:size}
\end{figure}
