...
def summarize(results: list[EpisodeResult], seed: int = 0) -> ConditionReport:
    ...
    benign_engagement = sum(r.benign_tool_calls for r in results) / sum(
        max(1, r.benign_steps) for r in results
    )
    ...
    report = ConditionReport(
        ...
        contagion=contagion,
        benign_engagement=benign_engagement,
    )
    for name, samples in (
